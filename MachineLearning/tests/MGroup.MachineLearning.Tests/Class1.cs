...
namespace MGroup.MachineLearning.Tests
{
	...
	/// </summary>
	public class DigitRecognitionCnnKeras
	{
		...
		Tensor accuracy(Tensor y_pred, Tensor y_true)
		{
			...
			var temp2 = tf.cast(y_true, tf.int64);
			var correct_prediction = tf.equal(tf.math.argmax(y_pred, 1), tf.cast(y_true, tf.int64));
			var temp = tf.reduce_mean(tf.cast(correct_prediction, tf.float32), axis: -1);
